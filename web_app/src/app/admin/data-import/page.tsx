    ...
    >
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="text-xs text-muted-foreground ml-auto">
          {t("admin.import.records_total")}:{" "}
          <strong>{report.recordsTotal}</strong>
        </span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <CountCard
          label={t("admin.import.created")}
          count={report.created}
          colorClass="text-green-600"
          bgClass="bg-green-500/10 border-green-500/20"
        />
        <CountCard
          label={t("admin.import.updated")}
          count={report.updated}
          colorClass="text-blue-600"
          bgClass="bg-blue-500/10 border-blue-500/20"
        />
        <CountCard
          label={t("admin.import.skipped")}
          count={report.skipped}
          colorClass="text-muted-foreground"
          bgClass="bg-muted/50 border-border/40"
        />
      </div>
...
export default function AdminDataImportPage() {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get<HistoryRecord[]>(
        API_ENDPOINTS.ADMIN.DATA_IMPORT.HISTORY,
      );
      setHistory(res.data ?? []);
    } catch (err) {
  ...
  const validateFile = (f: File): string | null => {
    ...
    if (!f.name.toLowerCase().endsWith(".json"))
      return t("admin.import.validation.invalid_type");
    return null;
  };
      ...
      if (isServiceUnavailable(err)) {
        ...
        const msg =
          (err as { response?: { data?: { error?: string } } })?.response?.data
            ?.error || t("admin.import.request_failed");
        setReport({
          type: selectedType,
      ...
      <AdminSectionCard title={t("admin.import.upload_title")}>
        ...
        >
          ...
          <p className="font-bold text-foreground">
            {t("admin.import.upload_drag")}
          </p>
          <p className="text-muted-foreground text-sm mt-1">
            {t("admin.import.upload_browse")}
          </p>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50 border-b border-border/40">
                <tr className="text-xs uppercase tracking-wide text-muted-foreground">
                  ...
                  <th className="px-4 py-3 text-left font-semibold">
                    {t("admin.import.history_date")}
                  </th>
                  <th className="px-4 py-3 text-left font-semibold">
                    {t("admin.import.history_user")}
                  </th>
                  <th className="px-4 py-3 text-left font-semibold">
                    {t("admin.import.history_type")}
                  </th>
                  <th className="px-4 py-3 text-left font-semibold">
                    {t("admin.import.history_file")}
                  </th>
                  <th className="px-4 py-3 text-left font-semibold">
                    {t("admin.import.history_mode")}
                  </th>
                  <th className="px-4 py-3 text-left font-semibold">
                    {t("admin.import.history_records")}
                  </th>
                  <th className="px-4 py-3 text-left font-semibold">
                    {t("admin.import.history_status")}
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border/30">
                {history.map((h) => {
                  ...
                  const totalRecords =
                    h.createdCount + h.updatedCount + h.skippedCount;
                  return (
                    <React.Fragment key={h.id}>
                      <tr
                        onClick={() =>
                          setExpandedRowId(isExpanded ? null : h.id)
                        }
                      ...
                      >
                        ...
                        <td
                          className="px-4 py-3 text-muted-foreground max-w-[140px] truncate text-xs"
                          title={h.fileName}
                        >
                          {h.fileName}
                        </td>
                        <td className="px-4 py-3">
                          ...
                          >
                            {h.dryRun
                              ? t("admin.import.history_preview")
                              : t("admin.import.history_import")}
                          </Badge>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          ...
                          >
                            {h.status === "SUCCESS"
                              ? `✅ ${t("admin.import.history_success")}`
                              : `❌ ${t("admin.import.history_failed")}`}
                          </Badge>
                        </td>
                      </tr>
                        ...
                        <tr className="bg-muted/20">
                          <td colSpan={8} className="px-6 py-4 space-y-3">
                            <div className="grid grid-cols-3 gap-3">
                              <CountCard
                                label={t("admin.import.created")}
                                count={h.createdCount}
                                colorClass="text-green-600"
                                bgClass="bg-green-500/10 border-green-500/20"
                              />
                              <CountCard
                                label={t("admin.import.updated")}
                                count={h.updatedCount}
                                colorClass="text-blue-600"
                                bgClass="bg-blue-500/10 border-blue-500/20"
                              />
                              <CountCard
                                label={t("admin.import.skipped")}
                                count={h.skippedCount}
                                colorClass="text-muted-foreground"
                                bgClass="bg-muted/50 border-border/40"
                              />
                            </div>
                            {h.warningSummary && (
                              <div className="space-y-1.5">
                                ...
                                <ul className="rounded-xl bg-amber-500/10 border border-amber-500/20 p-3 text-xs text-amber-700 space-y-0.5 max-h-32 overflow-y-auto">
                                  {h.warningSummary.split("; ").map((w, i) => (
                                    <li key={i}>• {w}</li>
                                  ))}
                                </ul>
                              </div>
