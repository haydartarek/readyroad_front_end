...
export function ScoreBreakdown({
  ...
  const tierConfig = {
    strong: {
      ...
      label: t("exam.breakdown.strong_badge"),
    },
    average: { bar: "", badge: "", label: "" },
    weak: {
      bar: "[&>div]:bg-red-500",
      badge: "bg-red-100 text-red-800 border-red-200",
      label: t("exam.breakdown.needs_practice"),
    },
  } as const;
  const summaryCells = [
    {
      tier: "strong" as Tier,
      label: t("exam.breakdown.strong"),
      color: "text-green-600",
    },
    {
      tier: "average" as Tier,
      label: t("exam.breakdown.average"),
      color: "text-orange-500",
    },
    {
      tier: "weak" as Tier,
      label: t("exam.breakdown.weak"),
      color: "text-red-600",
    },
  ];
  ...
  const tierCounts = useMemo(
    () => ({
      strong: categoryBreakdown.filter(
        (c) => getTier(c.percentage) === "strong",
      ).length,
      average: categoryBreakdown.filter(
        (c) => getTier(c.percentage) === "average",
      ).length,
      weak: categoryBreakdown.filter((c) => getTier(c.percentage) === "weak")
        .length,
    }),
    [categoryBreakdown],
  );
