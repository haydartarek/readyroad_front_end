...
const TYPE_I18N: Record<string, string> = {
  ...
  ADMIN_NEW_USER: "notif.type.admin_new_user",
  ADMIN_EXAM_STATS: "notif.type.admin_exam_stats",
  ADMIN_PLATFORM_ALERT: "notif.type.admin_alert",
  LESSON_PROGRESS: "notif.type.lesson_progress",
  NEXT_STEP: "notif.type.next_step",
};
...
export function NotificationPanel() {
  ...
  const timeAgo = useCallback(
    (iso: string): string => {
      ...
      if (diff < 3600)
        return t("notif.time_minutes").replace(
          "{n}",
          String(Math.floor(diff / 60)),
        );
      if (diff < 86400)
        return t("notif.time_hours").replace(
          "{n}",
          String(Math.floor(diff / 3600)),
        );
      return t("notif.time_days").replace(
        "{n}",
  ...
  return (
    <div ref={panelRef} className="relative">
      ...
      <Button
        ...
        aria-label={
          unreadCount > 0
            ? `${t("notif.title")} (${unreadCount} ${t("notif.unread_label")})`
            : t("notif.title")
        }
      ...
      >
        <Bell
          className={cn(
            ...
            unreadCount > 0 && !isOpen && "animate-[wiggle_1s_ease-in-out]",
          )}
        />
        {unreadCount > 0 && (
          <>
            {/* Pulse ring */}
            <span className="absolute -right-0.5 -top-0.5 h-[18px] w-[18px] rounded-full bg-red-500/30 animate-ping" />
            {/* Badge */}
            <span
              className={cn(
                "absolute -right-1 -top-1 flex min-w-[18px] h-[18px] items-center justify-center",
                "rounded-full bg-gradient-to-br from-red-500 to-rose-600",
                "px-1 text-[10px] font-black leading-none text-white",
                "shadow-md shadow-red-500/40 ring-[1.5px] ring-background",
              )}
            >
              {badgeCount}
            </span>
                  ...
                  const displayMessage = (() => {
                    ...
                    try {
                      const params = notif.messageParams
                        ? (JSON.parse(notif.messageParams) as Record<
                            string,
                            string | number
                          >)
                        : undefined;
                  ...
                  const content = (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p
                          dir="auto"
                          className="mt-0.5 text-xs leading-snug text-muted-foreground line-clamp-2"
                        >
                          {displayMessage}
                        </p>
