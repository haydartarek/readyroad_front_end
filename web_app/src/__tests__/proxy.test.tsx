...
describe("Middleware Logic", () => {
  ...
  test("JWT format validation rejects invalid tokens", () => {
    ...
    expect(
      isValidTokenFormat("eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ0ZXN0In0.sig"),
    ).toBe(true);
  });
  ...
  test("returnUrl validation only allows internal paths", () => {
    expect("/dashboard".startsWith("/") && !"/dashboard".startsWith("//")).toBe(
      true,
    );
    expect("//evil.com".startsWith("/") && !"//evil.com".startsWith("//")).toBe(
      false,
    );
    expect(
      "https://evil.com".startsWith("/") &&
        !"https://evil.com".startsWith("//"),
    ).toBe(false);
  });
