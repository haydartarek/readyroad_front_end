...
export default function AdminUsersPage() {
    ...
    const toggleLock = async (user: UserRow) => {
        ...
        try {
            ...
            logApiError('Failed to toggle user lock', e);
            // Rollback
    ...
    const changeRole = async (user: UserRow, newRole: string) => {
        ...
        try {
            ...
            logApiError('Failed to change user role', e);
            // Rollback
    ...
    return (
        <div className="space-y-6">
            {serviceUnavailable && <ServiceUnavailableBanner onRetry={() => fetchUsers(page)} className="mb-4" />}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div className="rounded-lg border bg-card p-4 shadow-sm">
                    <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t('admin.users.total_users')}</p>
            ...
            <div className="flex items-center gap-3">
                <div className="relative flex-1 max-w-md">
                    ...
                    <input
                        ...
                        className="w-full rounded-lg border border-border py-2 pl-10 pr-4 text-sm focus:border-ring focus:ring-1 focus:ring-ring outline-none"
                    />
                </div>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? (
                                ...
                                filteredUsers.map(user => {
                                    ...
                                    return (
                                        <tr key={user.id} className={`hover:bg-muted transition-colors ${isLocked ? 'bg-red-50/30' : ''}`}>
                                            {/* User */}
                                            <td className="px-4 py-3">
                                                <div className="flex items-center gap-3">
                                                    <div className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold text-white ${user.role === 'ADMIN' ? 'bg-purple-500' :
                                                        user.role === 'MODERATOR' ? 'bg-blue-500' : 'bg-muted-foreground'
                                                        }`}>
                                            ...
                                            <td className="px-4 py-3">
                                                <select
                                                    ...
                                                    disabled={isLoading}
                                                    className={`rounded-md border px-2 py-1 text-xs font-medium outline-none focus:ring-1 focus:ring-ring ${user.role === 'ADMIN' ? 'border-purple-200 bg-purple-50 text-purple-700' :
                                            ...
                                            <td className="px-4 py-3">
                                                <div className="flex flex-col gap-1">
                                                    <span className={`inline-flex w-fit items-center rounded-full px-2 py-0.5 text-xs font-medium ${user.isActive ? 'bg-green-50 text-green-700' : 'bg-muted text-muted-foreground'
                                                        }`}>
                    ...
                    <div className="flex items-center justify-between border-t bg-muted px-4 py-3">
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className="rounded-md border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-accent disabled:opacity-40"
                            >
                                {t('exam.previous')}
                            </button>
                            <button
