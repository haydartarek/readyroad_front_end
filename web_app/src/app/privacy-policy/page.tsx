...
interface PolicyContent {
  ...
  intro: string; sections: PolicySection[];
}
...
For data protection inquiries, you may also contact the Belgian Data Protection Authority (GBA).`
  ...
  nl: {
    ...
    sections: [
      { title: '1. Informatie die wij verzamelen',    content: 'Wij verzamelen informatie die u direct aan ons verstrekt: account informatie (e-mail, gebruikersnaam), voortgangsgegevens (examenuitslagen, oefentests), voorkeuren en gebruiksgegevens.' },
      { title: '2. Hoe wij uw informatie gebruiken',  content: 'Wij gebruiken de verzamelde informatie voor het leveren van de ReadyRoad service, het bijhouden van uw voortgang, personalisatie van uw leerervaring en verbetering van onze diensten.' },
      { title: '3. Gegevensopslag en beveiliging',    content: 'Alle gegevens worden versleuteld verzonden via HTTPS/TLS. Wachtwoorden worden gehasht en nooit in platte tekst opgeslagen. Wij voldoen aan de GDPR en Belgische gegevensbeschermingswetten.' },
      { title: '4. Contact',                          content: 'Voor privacyvragen: privacy@readyroad.be' },
    ]
  },
  fr: {
    ...
    sections: [
      { title: '1. Informations que nous collectons',        content: "Nous collectons les informations que vous nous fournissez directement : informations de compte (email, nom d'utilisateur), données de progression (résultats d'examen, tests pratiques), préférences et données d'utilisation." },
      { title: '2. Comment nous utilisons vos informations', content: "Nous utilisons les informations collectées pour fournir le service ReadyRoad, suivre vos progrès, personnaliser votre expérience d'apprentissage et améliorer nos services." },
      { title: '3. Stockage et sécurité des données',        content: 'Toutes les données sont cryptées en transit via HTTPS/TLS. Les mots de passe sont hachés et jamais stockés en texte clair. Nous respectons le RGPD et les lois belges sur la protection des données.' },
      { title: '4. Contact',                                 content: 'Pour les questions de confidentialité : privacy@readyroad.be' },
    ]
  },
  ar: {
    ...
    sections: [
      { title: '1. المعلومات التي نجمعها',  content: 'نجمع المعلومات التي تقدمها لنا مباشرة: معلومات الحساب (البريد الإلكتروني، اسم المستخدم)، بيانات التقدم (نتائج الامتحان، الاختبارات التدريبية)، التفضيلات وبيانات الاستخدام.' },
      { title: '2. كيف نستخدم معلوماتك',    content: 'نستخدم المعلومات المجمعة لتوفير خدمة ReadyRoad، وتتبع تقدمك، وتخصيص تجربة التعلم الخاصة بك، وتحسين خدماتنا.' },
      { title: '3. تخزين البيانات والأمان',  content: 'يتم تشفير جميع البيانات أثناء النقل عبر HTTPS/TLS. يتم تجزئة كلمات المرور ولا يتم تخزينها أبدًا بنص عادي. نحن نلتزم بقانون GDPR وقوانين حماية البيانات البلجيكية.' },
      { title: '4. اتصل بنا',               content: 'لأسئلة الخصوصية: privacy@readyroad.be' },
    ]
  },
};
  ...
  return (
    <div className="container max-w-4xl py-10 px-4" dir={isRTL ? 'rtl' : 'ltr'}>
      <Card className="rounded-2xl border-border/50 shadow-sm overflow-hidden">
        {/* Header */}
        <CardHeader className="bg-muted/50 border-b border-border/40 space-y-2 px-6 py-6">
