 ...
 *
 * This eliminates the need for localStorage token storage entirely.
 */
...
async function proxyRequest(
    ...
    if (MUTATION_METHODS.has(request.method)) {
        if (contentType?.includes('multipart/form-data')) {
            // For file uploads: forward raw body, remove Content-Type to let
            // fetch auto-set boundary. We need to re-create the FormData.
    ...
    try {
        ...
        // Distinguish connection errors (backend down) from unexpected failures
